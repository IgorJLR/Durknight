...
func ready():
	if c == true:
		...
		eventos["Checkpoint"] = "Cidade Deserto"
...
func interact():
	if interacao == true:
		...
		if positivo == true and olhando[0] == true and olhando[1] == "Skeet" or positivo == true  and pointer == "diálogo skeet":
				$MenuMensagem.set_visible(true)
				permition = false
				$MenuMensagem/LabelMensagem.set_visible(true)
				if textindex == 0:
					...
					if eventos["principal"] <= 7:
						textprep()
						textanimation("Esses magos de gelo imbecis, todos sabem que foi um deles que matou o rei,",0,true,"",0,"",0)
					else:
						textprep()
						textanimation("Eu escutei uma grande explosão vinda do leste. O que será que aconteceu?",0,true,"",0,"",0)
				...
				if textindex == 1:
					...
					if eventos["principal"] <= 7:
						textprep()
						textanimation("sempre se acharam melhores que todos.",1,true,"",0,"",0)
					else:
						textprep()
						textanimation("Foi você? Demônios? Pergaminho? Bruh",1,true,"",0,"",0)
				movimento = false
				pointer = "diálogo skeet"
				interacao = false
		...
		if positivo == true and olhando[0] == true and olhando[1] == "Loyce" or positivo == true  and pointer == "diálogo loyce":
				...
				if textindex == 0:
					...
					if eventos["principal"] <= 7:
						...
						textprep()
						textanimation("O povo unido poderá derrotar qualquer inimigo, no deserto temos o aço e os",0,true,"",0,"",0)
				...
				if textindex == 1:
					...
					if eventos["principal"] <= 7:
						...
						textprep()
						textanimation("mestres ferreiros, do campo os recursos, mas ainda falta algo...",1,true,"",0,"",0)
				movimento = false
		...
		if positivo == true and olhando[0] == true and olhando[1] == "John" or positivo == true  and pointer == "diálogo john":
				...
				if textindex == 0:
					...
					if eventos["principal"] <= 7:
						...
						textprep()
						textanimation("Vejam seus moleques vocês terão que trabalhar muito para compensar o que",0,true,"",0,"",0)
				...
				if textindex == 1:
					...
					if eventos["principal"] <= 7:
						...
						textprep()
						textanimation("fizeram! Agora quero ver as armas brilhando, e nada de reclamações!!!",1,true,"[shake rate=30 level=2]",45,"",0)
				if textindex == 2:
					$MenuMensagem/LabelMensagem.set_use_bbcode(true)
					$MenuMensagem/LabelMensagem.bbcode_text = ""
					if eventos["principal"] <= 7:
						textprep()
						textanimation("Ninguém parece se importar com mais nada.",2,true,"",0,"",0)
					else:
						textprep()
						textanimation("Obrigado por ter aberto nossos olhos " + eventos["Nome"]+".",1,true,"",0,"",0)
				movimento = false
		...
		if positivo == true and olhando[0] == true and olhando[1] == "Shanalotte" or positivo == true  and pointer == "diálogo shanalotte":
				...
				if textindex == 0:
					...
					if eventos["principal"] <= 7:
						...
						textprep()
						textanimation("Por favor, traga os magos de gelo de volta ao juizo, ouvi dizer que sem",0,true,"",0,"",0)
				...
				if textindex == 1:
					...
					if eventos["principal"] <= 7:
						...
						textprep()
						textanimation("piedade estão usando sua magia para transformar as pessoas que",1,true,"",0,"",0)
				if textindex == 2:
					$MenuMensagem/LabelMensagem.set_use_bbcode(true)
					$MenuMensagem/LabelMensagem.bbcode_text = ""
					if eventos["principal"] <= 7:
						...
						textprep()
						textanimation("vão até o bioma de gelo em criaturas brancas estranhas.",1,true,"",0,"",0)
				movimento = false
...
func continuar():
	...
	if voltar == true and pointer == "loja Nemo":
		...
		if eventos["principal"] <= 7:
			...
			textprep()
			textanimation("Se cuide. A propósito bela vitória.",2,true,"",0,"",0)
		#-52 -23
		$escMenu/equipados/RichTextLabel.set_position(Vector2(-52, -23))
		$escMenu/Sprite.set_visible(false)
		$escMenu/equipados/RichTextLabel.set_visible(false)
...
func _on_Exit_body_entered(body):
	if body.get_name() == "personagemdserto":
		go("Mundo") 
