...
var nemo = {"Poção de Vida Média":[9],"Poção de Mana Média":[9], "Elmo de Aço":[1], "Peitoral de Aço":[1], "Espada de Soldado":[1],"Perneiras de Aço":[1]}
...
func ready():
	if c == true:
		...
		eventos["Checkpoint"] = "Cidade Gelo"
...
func interact():
	if interacao == true:
		...
		if positivo == true and olhando[0] == true and olhando[1] == "Gerald" or positivo == true  and pointer == "diálogo gerald":
			$MenuMensagem.set_visible(true)
			permition = false
			$MenuMensagem/LabelMensagem.set_visible(true)
			if textindex == 0 and gerald == true:
				$MenuMensagem/LabelMensagem.bbcode_text = ""
				if eventos["principal"] <= 2:
					$MenuMensagem/LabelMensagem.bbcode_text += "Sem dinheiro não passa!"
					textprep()
					textanimation("Sem dinheiro não passa!",0,true,"",0,"",0)
				else:
					$MenuMensagem/LabelMensagem.bbcode_text += "Aquele garoto está um palito, essa situação toda é muito revoltante."
					textprep()
					textanimation("Aquele garoto está um palito, essa situação toda é muito revoltante.",0,true,"",0,"",0)
			if textindex == 0 and gerald == false:
				gerald = true
				$MenuMensagem/LabelMensagem.bbcode_text = ""
				if eventos["principal"] <= 2:
					$MenuMensagem/LabelMensagem.bbcode_text += "Quer passar é? Primeiro vou querer 1000 moedas HAHAHAHA!"
					textprep()
					textanimation("Quer passar é? Primeiro vou querer 1000 moedas HAHAHAHA!",0,true,"",0,"",0)
				else:
					$MenuMensagem/LabelMensagem.bbcode_text += "Demônios? Nunca ouvi falar, por que fizeram isso com nosso reino?"
					textprep()
					textanimation("Demônios? Nunca ouvi falar, por que fizeram isso com nosso reino?",0,true,"",0,"",0)
			...
			if textindex == 1:
				...
				if eventos["principal"] <= 2:
					textindex+= 0.5
					$MenuMensagem/LabelMensagem.bbcode_text = "(ele bloqueia a passagem)"
				else:
					$MenuMensagem/LabelMensagem.bbcode_text = "Se fosse por mim correria direto para aquele rei demônio e acabaria com ele."
					textprep()
					textanimation("Se fosse por mim correria direto para aquele rei demônio e acabaria com ele.",1,true,"",0,"",0)
			movimento = false
		...
		if positivo == true and olhando[0] == true and olhando[1] == "Mago de Gelo B" or positivo == true  and pointer == "diálogo B":
				$MenuMensagem.set_visible(true)
				permition = false
				$MenuMensagem/LabelMensagem.set_visible(true)
				if textindex == 0:
					...
					if eventos["principal"] <= 9:
						...
						textprep()
						textanimation("Meu Deus! Você viu aquilo?!",0,true,"",0,"",0)
		...
		if positivo == true and olhando[0] == true and olhando[1] == "John" or positivo == true  and pointer == "diálogo john":
				$MenuMensagem.set_visible(true)
				permition = false
				$MenuMensagem/LabelMensagem.set_visible(true)
				if textindex == 0:
				...
				if textindex == 1:
					...
					if eventos["principal"] <= 5:
						textprep()
						textanimation("próxima pessoa para roubar, e ninguém faz nada.",1,true,"",0,"",0)
					else:
						textprep()
						textanimation("",1,true,"",0,"",0)
				if textindex == 2:
					...
					if eventos["principal"] <= 5:
						textprep()
						textanimation("Ninguém aqui parece se importar com mais nada.",2,true,"",0,"",0)
					else:
						textprep()
						textanimation("",1,true,"",0,"",0)
				movimento = false
		...
		if positivo == true and olhando[0] == true and olhando[1] == "Shanalotte" or positivo == true  and pointer == "diálogo shanalotte":
				$MenuMensagem.set_visible(true)
				permition = false
				$MenuMensagem/LabelMensagem.set_visible(true)
				if textindex == 0:
				...
				if textindex == 1:
					...
					if eventos["principal"] <= 5:
						textprep()
						textanimation(" magia para deixar a vida no deserto suportável,",1,true,"",0,"",0)
					else:
						textprep()
						textanimation("",1,true,"",0,"",0)
				if textindex == 2:
					...
					if eventos["principal"] <= 5:
						textprep()
						textanimation("sem eles a vida aqui parece um inferno!",2,true,"",0,"",0)
					else:
						textprep()
						textanimation("",1,true,"",0,"",0)
				movimento = false
		...
		if positivo == true and olhando[0] == true and olhando[1] == "lootCasa2" or positivo == true  and pointer == "lootCasa2":
			...
			if textindex == 1:
				$MenuMensagem/LabelMensagem.set_bbcode("Não tem mais nada.")
		if positivo == true and olhando[0] == true and olhando[1] == "Nemo" or positivo == true  and pointer == "loja Nemo" or switchh == 5 or force == true:
			if switchh == 5:
				switchh = 6
				textindex = 1
			$MenuMensagem.set_visible(true)
			...
			if textindex == 0:
				if eventos["principal"] < 4:
					...
					textprep()
					textanimation("Porque estou num barril? Tente forjar armas nesse calor e saberá!",0,true,"",0,"",0)
				else:
...
func continuar():
	...
	if voltar == true and pointer == "loja Nemo":
		...
		textprep()
		textanimation("Se cuide.",2,true,"",0,"",0)
		#-52 -23
		$escMenu/equipados/RichTextLabel.set_position(Vector2(-52, -23))
		$escMenu/Sprite.set_visible(false)
		$escMenu/equipados/RichTextLabel.set_visible(false)
...
func _physics_process(delta):
	som()
	print(eventos["principal"])
	ready()
	olhando = $"../Mago de Gelo A".olhando
	olhando = $"../Mago de Gelo B".olhando
	olhando = $"../Mago de Gelo C".olhando
	if eventos["principal"] < 10:
		olhando = $"../Mago de Gelo D".olhando
	continuar()
	bumper(direcao)
	get_input()
...
func _on_Exit_body_entered(body):
	if body.get_name() == "personagemgelo":
		if eventos["principal"]  == 11:
			go("Join")
		else:
			go("Mundo")
